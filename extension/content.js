// AgentOS Bridge - Content Script v2.0.1
// Fixes: stronger dedup for TASK_DONE, 60s window, node-level dedup
(function() {
  'use strict';
  ...
  function isInsideAssistantMessage(node) {
    ...
    while (el && el !== document.body) {
      ...
      if (el.classList && (el.classList.contains('response-content') ||
        el.classList.contains('bot-message') || el.classList.contains('assistant-message'))) return true;
      el = el.parentElement;
    }
    return false;
  }
  ...
  function init() {
    ...
    console.log('[AgentOS v2.0.1] Content script loaded - Tools Engine active');
  }
